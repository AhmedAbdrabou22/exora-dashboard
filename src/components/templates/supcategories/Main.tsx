...
function MainSupCategories() {
  ...
  const columns = useMemo(
    () => generateColumns({ refetch, setIsModalOpen, setMainData }),
    [refetch]
  );

  const handlePageChange = ({ selected }: { selected: number }) => {
    setPage(selected + 1); // react-paginate uses 0-based pages
  };
...
export default MainSupCategories;
